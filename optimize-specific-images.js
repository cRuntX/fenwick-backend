/**
 * AGGRESSIVE OPTIMIZATION - SPECIFIC IMAGES ONLY
 */
...
// Aggressive settings for selected images
const MAX_WIDTH = 600;
const MAX_HEIGHT = 400;
const QUALITY = 60;

// Images to aggressively optimize (partial name matching)
const TARGET_IMAGES = [
  ...
  'Spielplatz',
  'Mühlbach',
  'Horses for the Historic Carriage',
  'Blue Porcelain',
  'ShangriLA',
  'Hilton'
];

console.log('🎯 Aggressive Optimization - Selected Images Only\n');
...
async function optimizeSelected() {
  try {
    ...
    console.log(`Targeting ${targetProjects.length} projects for aggressive optimization\n`);

    console.log('📋 Projects to optimize:');
    ...
    for (let i = 0; i < targetProjects.length; i++) {
      ...
      try {
        ...
        console.log(`✅ ${num} ${project.name.substring(0, 40).padEnd(40)} ${originalSizeKB.toFixed(1).padStart(6)} KB → ${optimizedSizeKB.toFixed(1).padStart(6)} KB (${reduction.toFixed(1)}%)`);
        processedCount++;
    ...
    console.log(`   ⏭️  Skipped: ${allProjects.length - targetProjects.length} images (not in target list)`);
...
optimizeSelected()
  .then(() => {
    console.log('✅ Selected images optimized!');
    process.exit(0);
  })
  .catch((err) => {
